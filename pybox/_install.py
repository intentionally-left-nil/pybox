...
def _write_python_launcher_env(
    ...
    To keep Python's tempfile module functional we redirect TMPDIR to venv/tmp,
    which is inside the sandbox's writable zone.
    """
    venv_str = str(venv_root)
    profile = f"{venv_str}/share/pybox/pybox_profile.json"
    jupyter_base = f"{venv_str}/share/jupyter"
    venv_tmp = f"{venv_str}/tmp"
    mkdir_dirs = ":".join([
...
def _write_pip_launcher_env(
    ...
    - PYBOX_MKDIR includes pip-cache and tmp so pip's HTTP/wheel cache and
      temp build dirs are always available inside the sandbox.
    ...
    - TMPDIR/TMP/TEMP are redirected to venv/tmp so pip's ephemeral wheel
      build directories go inside the sandbox rather than to /var/folders.
    ...
    content = "\n".join([
        ...
        f"PYBOX_MKDIR={venv_str}/__pycache__:{venv_str}/pip-cache:{venv_tmp}",
        f"PYBOX_ENV_PYTHONPYCACHEPREFIX={venv_str}/__pycache__",
        f"PYBOX_ENV_TMPDIR={venv_tmp}",
        f"PYBOX_ENV_TMP={venv_tmp}",
        f"PYBOX_ENV_TEMP={venv_tmp}",
...
def _build_python_profile(venv_root: Path, python_exe_names: list[str]) -> dict:
    ...
    - Includes python_runtime so the nosandbox interpreter is readable
      regardless of where it lives (Homebrew, uv, pyenv, conda, etc.).
    ...
    - /tmp and /private/tmp are explicitly denied to override the system_write_macos
      group grant. /var/folders and /private/var/folders (where $TMPDIR normally
      resolves on macOS) are also denied. The launcher redirects TMPDIR/TMP/TEMP
      to venv/tmp so Python's tempfile module still works inside the sandbox.
    """
    venv_str = str(venv_root)
    ...
    # Deny all system temp directories to enforce write-only-to-CWD.
    # /tmp → /private/tmp on macOS; /var/folders and /private/var/folders cover
    # $TMPDIR (the per-user temp dir). The launcher redirects TMPDIR to venv/tmp.
    deny_list = protected_bin + [
        "/tmp", "/private/tmp",
    ...
    return {
        "meta": {
            ...
            "description": (
                "pybox python sandbox: read venv, write only to venv/__pycache__ and venv/tmp, "
                "read+write $WORKDIR, deny writes to venv/bin python executables, pybox launcher, and tmp paths"
            ),
        },
        "extends": "default",
        "groups": {
...
def _build_pip_profile(venv_root: Path, python_exe_names: list[str]) -> dict:
    ...
    - lib, lib64, include, share, __pycache__, pip-cache, tmp are all writable
      for normal package installation. tmp is required for pip's ephemeral wheel
      build directories (TMPDIR is redirected there by the launcher).
    - /tmp, /private/tmp, /var/folders, /private/var/folders are denied to
      prevent pip post-install hooks from writing outside the venv.
    """
    venv_str = str(venv_root)
    ...
    return {
        "meta": {
            ...
            "description": (
                ...
                "deny writes to venv/bin python executables, pybox launcher, and tmp paths"
            ),
        },
        "extends": "default",
        "groups": {
